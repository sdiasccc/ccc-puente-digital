...
export default function ComunicacionesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {active.length === 0 ? (
        <EmptyState icon={<Megaphone className="h-12 w-12" />} title="Sin comunicados" description="No hay comunicados publicados actualmente" />
      ) : (
        <div className="space-y-4">
          {active.map((comm) => (
            <div key={comm.id} className="rounded-xl border bg-card p-6 card-shadow hover:card-shadow-hover transition-all">
